...
function isFloridaLaunch(l){
  ...
  // Accept when country is missing; only exclude if country is present and not US
  ...
  const floridaKeywords = /(cape canaveral|kennedy|patrick|merritt island|canaveral|cocoa)/i;
  return /florida/i.test(combined) || /,\s*fl\b/i.test(combined) || /(cape canaveral|kennedy|patrick|merritt island|canaveral)/i.test(combined) || floridaKeywords.test(combined);
}
